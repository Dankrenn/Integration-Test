...
using Донских_4_финал.Интерфейсы;
...
namespace integtest.Classes
{
    public class TriangleValidateService : ITriangleValidateService
    {
        ...
        public bool IsValid(int id) //ПРОВЕРКА НА ВОЗМОЖНОСТЬ СОЗДАНИЯ ПО АЙДИ
        {
            Triangle triangle = TriangleProvider.GetById(id);//ВЫТЯГИВАЕМ ТРЕУГОЛЬНИК ИЗ БАЗЫ
            bool bools = true;
